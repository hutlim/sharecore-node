...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/sharecore-node.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update sharecore-node.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'sharecore-lib': '^v0.1.0',
          'sharecore-node': '^v0.2.0'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/sharecore-node.json');
