...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'sharecore-lib': '^' + bitcore.version,
    'sharecore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/sharecore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the sharecore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the sharecore-node directory and configuration
